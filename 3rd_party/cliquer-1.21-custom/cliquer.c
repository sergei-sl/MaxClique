...
#define IN_EPS(a, b) (fabs(a-b) < 0.00001)
...
static float *clique_size;      /* c[i] == max. clique size in {0,1,...,i-1} */
...
static float weight_multiplier=1.0;  /* Weights multiplied by this when passing
				  * to time_function(). */
...
static boolean sub_unweighted_single(int *table, int size, float min_size,
				     graph_t *g);
static int sub_unweighted_all(int *table, int size, float min_size, float max_size,
			      boolean maximal, graph_t *g,
			      clique_options *opts);
static float sub_weighted_all(int *table, int size, float weight,
			    float current_weight, float prune_low, float prune_high,
			    float min_weight, float max_weight, boolean maximal,
 ...
 */
static float unweighted_clique_search_single(int *table, float min_size,
					   graph_t *g, clique_options *opts) {
	int i,j;
	int v,w;
	int *newtable;
...
static boolean sub_unweighted_single(int *table, int size, float min_size,
	...
	for (i = size-1; i >= 0; i--) {
		...
		if (sub_unweighted_single(newtable,(int)(p1-newtable),
					  min_size-1,g)) {
			/* Clique found. */
			SET_ADD_ELEMENT(current_clique,v);
			temp_list[temp_count++]=newtable;
...
static int unweighted_clique_search_all(int *table, int start,
	...
	for (i=start; i < g->n; i++) {
		v=table[i];
		clique_size[v]=(float)min_size;  /* Do not prune here. */
		...
		SET_ADD_ELEMENT(current_clique,v);
		j=sub_unweighted_all(newtable,newsize,(float)(min_size-1),(float)(max_size-1),
				     maximal,g,opts);
...
static int sub_unweighted_all(int *table, int size, float min_size, float max_size,
	...
	for (i=size-1; i>=0; i--) {
		...
		SET_ADD_ELEMENT(current_clique,v);
		n=sub_unweighted_all(newtable,(int)(p1-newtable),
 ...
 * non-NULL.
 *
 * Returns -1 if a clique of requested weight was not found (also if
 ...
 * Note: Does NOT use opts->user_function of opts->clique_list.
 */
static float weighted_clique_search_single(int *table, float min_weight,
					 float max_weight, graph_t *g,
					 clique_options *opts) {
	int i,j;
	int v;
	int *newtable;
	...
	temp_list[temp_count++]=newtable;
	if (!IN_EPS(min_weight, 0) && (search_weight > 0)) {
...
static int weighted_clique_search_all(int *table, int start,
	...
	for (i=start; i < g->n; i++) {
		...
		SET_ADD_ELEMENT(current_clique,v);
		wt=sub_weighted_all(newtable,newsize,newweight,
				   g->weights[v],min_weight-1,FLT_MAX,
...
static float sub_weighted_all(int *table, int size, float weight,
	...
	for (i = size-1; i >= 0; i--) {
		...
		if (current_weight+wt+newweight <= prune_low) {
			continue;
		}

		SET_ADD_ELEMENT(current_clique,v);
		prune_low=sub_weighted_all(newtable,(int)(p1-newtable),
					   newweight,
					   current_weight+wt,
 ...
 */
float clique_unweighted_max_weight(graph_t *g, clique_options *opts) {
	set_t s;
	int size;
 ...
 */
set_t clique_unweighted_find_single(graph_t *g,float min_size,float max_size,
	...
	clique_size=malloc(g->n * sizeof(float));
	...
	if (unweighted_clique_search_single(table,(float)min_size,g,opts)==0) {
 ...
 * by set_free().
 */
int clique_unweighted_find_all(graph_t *g, float min_size, float max_size,
			       boolean maximal, clique_options *opts) {
	int i;
	int *table;
	int count;
	...
	if (min_size==0 && max_size==0) {
		max_size=min_size=clique_size[table[g->n - 1]];
 ...
 *       This incurs only very small overhead.
 */
float clique_max_weight(graph_t *g,clique_options *opts) {
	set_t s;
    float weight;
 ...
 */
set_t clique_find_single(graph_t *g,float min_weight, float max_weight,
			 boolean maximal, clique_options *opts) {
	int i;
	int *table;
	set_t s;
	...
	ASSERT(min_weight>=0);
	ASSERT(max_weight>=0);
	ASSERT(IN_EPS(max_weight,0) || (min_weight <= max_weight));
	ASSERT(!((IN_EPS(min_weight,0)) && (max_weight>0)));
	ASSERT((opts->reorder_function==NULL) || (opts->reorder_map==NULL));
	...
	if (!graph_weighted(g)) {
		min_weight=min_weight/g->weights[0];
		if (max_weight) {
			max_weight=max_weight/g->weights[0];
	...
	clique_size=malloc(g->n * sizeof(float));
	memset(clique_size, 0, g->n * sizeof(float));
	/* table allocated later */
	temp_list=malloc((g->n+2)*sizeof(int *));
	temp_count=0;
 ...
 *       weights are the same.
 */
int clique_find_all(graph_t *g, float min_weight, float max_weight,
		    boolean maximal, clique_options *opts) {
    int i, n;
    float wt;
	int *table;
	...
	/* First phase */
	wt=weighted_clique_search_single(table,min_weight,FLT_MAX,g,opts);
	if (wt<0) {
		/* Requested clique has not been found. */
		goto cleanreturn;
	}
